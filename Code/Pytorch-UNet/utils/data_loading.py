...
class CustomDataset(Dataset):
    ...
    @staticmethod
    def preprocess(data, target, scale):
        # Assuming both data and target are numpy arrays
        new_size = int(scale * data.shape[0])
        assert new_size > 0, 'Scale is too small, resized arrays would have no elements'

        data_resized = np.resize(data, (new_size, new_size, data.shape[2]))
        target_resized = np.resize(target, new_size)

        return data_resized, target_resized
